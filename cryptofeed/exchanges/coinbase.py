...
def get_private_parameters(
    ...
    jwt_token = _build_jwt(
        config["coinbase"]["key_id"], config["coinbase"]["key_secret"], uri
    )
...
class Coinbase(Feed, CoinbaseRestMixin):
    ...
    def _get_auth_headers(self, config: Config = None) -> dict:
        if (
            ...
            raise ValueError(
                f"You must provide key_id and key_secret in config to retrieve symbols from Coinbase. config: {config}"
            )
